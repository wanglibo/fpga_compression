...
int deflate259_opencl(char* input, int in_len, char* tree,
    int tree_len, char* output, int* out_len);
...
int main(int argc, char **argv)
{
  ...
  char *treeBuf = (char*)mallocOrDie(TREE_MAX);
  ...
  treeByteSize = openReadOrDie(treeFilePath, treeBuf, TREE_MAX);
  treeBitSize = 8*(treeByteSize-2)+treeBuf[treeByteSize-1];

  // Execute api
  int outSize, goldenSize;
  double start = get_current_msec();
  int status = deflate259_opencl(inBuf, inSize, treeBuf, treeBitSize,
      outBuf, &outSize);
